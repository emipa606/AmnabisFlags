...
namespace Amnabi
{
    public class PR_QQHVerticalSplit : PR_VerticalSplit
    {
        public override void iterate(FactionFlagTags fft, PatternLayer layerNow, int depth)
        {
            ...
            ExecRandOrder(delegate
            {
                layerNow.childLayer(this,
                        new Rect(layerNow.innerrect.x, layerNow.innerrect.y, layerNow.innerrect.width / 4f,
                            layerNow.innerrect.height), depth).tagInc("SplitSub", 1).tagInc("SplitSubVert", 1)
                    .iterateLayer(fft, depth);
            }, delegate
            {
                layerNow.childLayer(this,
                        new Rect(layerNow.innerrect.x + (layerNow.innerrect.width * 1f / 4f), layerNow.innerrect.y,
