...
public class PR_TriangularLeftSplit : PR_Split
{
    ...
    public override void iterate(FactionFlagTags fft, PatternLayer layerNow, int depth)
    {
        ...
        layerNow.attachPre(layerNow.createFlagPattern(AmnabiFlagDefOfs.Pattern_TriangleA, choosen,
            layerNow.innerrect.width, layerNow.innerrect.height, 0.5, 1.0, 0.0));
        ExecRandOrder(delegate
        {
            layerNow.childLayer(this,
                    new Rect(layerNow.innerrect.x, layerNow.innerrect.y, layerNow.innerrect.width / 2f,
                        layerNow.innerrect.height / 2f), depth).tagInc("SplitSub", 1).tagInc("SplitSubVert", 1)
