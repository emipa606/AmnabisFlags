...
public class FactionFlagTags : IExposable
{
    ...
    public void initialize(Faction faction)
    {
        ...
        var factionFlg = FlagsCore.CreateFlagFaction(faction);
        factionFlg.patternStack.AddRange(list);
        factionFlg.patternMax = list.Count;
        Rand.PopState();
    }
    ...
    public Flag initialize(Settlement stt)
    {
        ...
        var list = new List<FlagPattern>();
        patternLayer.compile(list);
        var settlement = FlagsCore.CreateFlagSettlement(stt);
        settlement.patternStack.AddRange(list);
        settlement.patternMax = list.Count;
        Rand.PopState();
        return settlement;
    }
